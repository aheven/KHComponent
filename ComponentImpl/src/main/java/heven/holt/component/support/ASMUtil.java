...
import heven.holt.component.application.IComponentHostApplication;
 ...
 * 通过 {@link heven.holt.component.ComponentUtil#genHostFragmentClassName(String)} 等方法来生成一个
 * 全类名, 然后根据反射去寻找对应的类. 寻找失败会打印日志或者忽略, 但是不会崩溃, 因为这个本来就是需要用户保证传递的
 * 名称是有效的.
 * 上面提到了, 反射去找对应的类, 其实一两次的反射我不会关心. 但是其实由于框架的设计目前有几大模块：
 * 1. 路由模块 {@link heven.holt.component.anno.RouterAnno}
 * 2. 路由降级模块 {@link heven.holt.component.anno.RouterDegradeAnno}
 * 3. Service 服务发现模块 {@link heven.holt.component.anno.ServiceAnno}
 * 4. 拦截器模块 {@link heven.holt.component.anno.InterceptorAnno}
 * 5. Fragment 模块 {@link heven.holt.component.anno.FragmentAnno}
 * 6. 以及最重要的生命周期模块 {@link heven.holt.component.anno.ModuleAppAnno}
...
public class ASMUtil {
     ...
     * {@link heven.holt.component.ComponentUtil#transformHostForClass(String)}
     ...
     */
    @Nullable
    public static IModuleLifecycle findModuleApplicationAsmImpl(@NonNull String host) {
        return null;
    }
}
